...
def repl(unigramCost, bigramCost, possibleFills, command=None):
    ...
    while True:
        ...
        # unigram cost of the line following this command
        # (not the sum of unigram costs of all words)
        # e.g., `ug a` returns the cost for "a" and `ug a b c` returns the cost for "a b c"
        elif cmd == 'ug':
            line = wordsegUtil.cleanLine(line)
            print unigramCost(line)

        # bigram cost of the last two words following this command
        elif cmd == 'bg':
            grams = tuple(wordsegUtil.words(line))
            prefix, ending = grams[-2], grams[-1]
            print bigramCost(prefix, ending)

        # smoothed unigram and bigram cost
        elif cmd == 'subg':
            smoothCost = wordsegUtil.smoothUnigramAndBigram(unigramCost, bigramCost, 0.2)
            grams = tuple(wordsegUtil.words(line))
            prefix, ending = grams[-2], grams[-1]
            print smoothCost(prefix, ending)
