...
class GradingAgent(Agent):
  def __init__(self, studentAgent, solutionAgents=[], alternativeDepthAgents = [], partialPlyBugAgents = []):
  ...
  def checkFailure(self, agentName):
    """
...
def comparison_checking(theirPac, ourPacOptions, agentName):
  ...
  ourpac = []
  alternative_depth_pacs = []
  partial_ply_bug_pacs = []
  layname = 'smallClassic'
  for depth in [2]:
    pac = GradingAgent(theirPac, ourpac, alternative_depth_pacs, partial_ply_bug_pacs)
    name = '%s (depth %d)' % (agentName, depth)
    stats = run(layname, pac, [DirectionalGhost(i + 1) for i in range(2)], name = name)
    if stats['timeouts'] > 0: timeout = True
    code = pac.checkFailure(name)
    if code > 0:
      totalSuboptimal += code
    elif code == -2:
      partialPlyBug = True
    elif code == -1:
      offByOne = True
...
def testq4():
  ...
  if firstTime:
      grader.setSide({"name": "Score",
                      "value": averageWinScore,
                      "order": "asc"})
  firstTime = False
